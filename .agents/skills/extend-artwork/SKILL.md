---
name: extend-artwork
description: Reimagine an image as a 16:9 landscape suitable for Haystack's daily artwork rotation. Use only when the user explicitly invokes $extend-artwork with an image path; run the repository's direct-provider outpainting script and save the result to HAYSTACK_IMAGE_DIR.
---
